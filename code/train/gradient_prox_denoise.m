function [db, g_cof_denoise] = gradient_prox_denoise(cof, split_z_init, a, rho, Training, Model, split_z_local, phi, d_phi, c)
...
% gradient g >> split_u^(t) >> RBF_i      [to accumulate]
    ...
    if(i==Model.numDenoiseLayers)
        g_split_z_local(:,:,i) = rho.*a;
    else
        tp = zeros(Training.imdims);
        for j = 1:Model.numFilters
            tp = tp + imfilter_warp(d_phi(:,:,j,i+1).*(imfilter_warp(g_split_z_local(:,:,i+1), Model.filters2D(:,:,j,i+1))), Model.filters2Dinv(:,:,j,i+1));
        end
        g_split_z_local(:,:,i) = g_split_z_local(:,:,i+1) - tp;               
    end    
    q = g_split_z_local(:,:,i); 
    clique_q = q(Training.cliques_of_circ_conv) ;
    
    if(i==1)
        tp1 = zeros(Training.imdims);
        for j = 1:Model.numFilters
             tp1 = tp1 + imfilter_warp(d_phi(:,:,j,i).*(imfilter_warp(q, Model.filters2D(:,:,j,i))), Model.filters2Dinv(:,:,j,i));
        end        
        db = q - tp1;
    end
