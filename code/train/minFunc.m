...
% %---------------------------------------------
% %---------------------------------------------
% if(Training.do_progress_plot)  
%     obj_values = [obj_values, obj_value]; 
%     image_errors = [image_errors, image_err];
%     totalIter = Model.numStages*Training.maxIter;   
%     figure(2);
%     if(length(obj_values)>1)
%       subplot(121), scatter(1:length(obj_values), obj_values, 'k.'), axis([1 max(totalIter,length(obj_values)) 0 max(obj_values)]); title('training loss'); 
%     end
%     if(length(image_errors)>1)
%       subplot(122), scatter(1:length(image_errors), image_errors, 'b.'), axis([1 max(totalIter,length(image_errors)) 0 image_errors(1)+1e-4]); title('norm(x-x_{gt})');
%     end
%     drawnow;
% end
% %---------------------------------------------
