function [xMat, psnr_mean] = Adam(sg, x0, nvis, nIter, idxSG, stepSize, beta1, beta2, epsilon)
...
% compute intermediate PSNR results
psnr_mean = zeros(1, nIter + 1); %+20161031
...
%figure, hold on;
%global Training
%cc = rand(Training.N, 3);
%fCurr = zeros(Training.N, nIter);
% Run optimisation
count = 1;
...
for i = 1 : 1 : nIter
    ...
    if(mod(i, 10)==1) 
        ...
        [~,psnr_mean(count)] = computePSNR();   
        fprintf('\n>>>>>>>>>>\n');
        fprintf('SGD iter %d, psnr mean %.4f.', i, psnr_mean(count));
        fprintf('\n>>>>>>>>>>\n');
        count = count + 1;
    end
    %---------------------------------------------------------------------------------
