...
parfor i = 1:Training.N
   tp = cof_single(i, :);
   if(isPreTraining)
       [f_single(i), g_single(i, :)] = loss_func_admm_deep_bp_ensemble_pretrain(tp(:), Training, Model, Data{i}); 
   else
       [f_single(i), g_single(i, :)] = loss_func_admm_deep_bp_ensemble(tp(:), Training, Model, Data{i}); 
   end
end
%------------------------------------------------------------------------------------------------------
