function [f, g] = loss_func_admm_deep_bp_ensemble_pretrain(cof, Training, Model, DataOne) 
% evaluate the objective and gradient of the loss func 
% for SINGLE image

%--------------------------------------------------------
% for pre-training: only consider the first stage
assert(Training.isPreTraining);
assert(Training.iter_training==2);
...
rho_array = zeros(Training.iter_training, 1); 
omega_array = zeros(Training.imdims(1), Training.imdims(2), Training.iter_training); 
...
x = zeros(Training.imdims(1), Training.imdims(2), Training.iter_training);
...
phi = zeros(Training.imdims(1), Training.imdims(2), Model.numFilters, Model.numDenoiseLayers,Training.iter_training);
d_phi = zeros(Training.imdims(1), Training.imdims(2), Model.numFilters, Model.numDenoiseLayers,Training.iter_training);
c = zeros(Model.numRBFs_trained, prod(Training.imdims), Model.numFilters, Model.numDenoiseLayers,Training.iter_training);
...
split_z(:,:,1) = x(:,:,1);  % initialization   
 ...
 for t = 2:Training.iter_training
    ...
    if(Training.PreTrain.layer==1) % the first layer
        split_z_init = x(:,:,t-1);
    else
        split_z_init = zeros(Training.imdims(1), Training.imdims(2));
        split_z_init(:, :) = DataOne.PreTrain.split_z_local;
    end
    
    [split_z_local(:,:,:,t), phi(:,:,:,:,t), d_phi(:,:,:,:,t), c(:,:,:,:,t)] = prox_denoise_pretrain(cof, split_z_init, Training, Model); 
        
    split_z(:,:,t) = split_z_local(:,:,currentLayer,t);
   
    % update x
    % compute new latent image estimate with current model parameter
     top =  top_part_fidelity + rho_array(t).*fft2(split_z(:,:,t));
...
for t = Training.iter_training:-1:2     
    ...
    % gradient g >> x^(t) >> lambda  [to accumulate]
     ...
     a = real(ifft2(fft2(g_x)./omega_array(:,:,t))); % (Omega^-1) * (g_x) 
     x_new = x(:,:,t);
     ...
     [tp3] = gradient_prox_denoise_pretrain(cof, split_z_init, a, rho_array(t), Training, Model, split_z_local(:,:,:,t), phi(:,:,:,:,t), d_phi(:,:,:,:,t), c(:,:,:,:,t));
